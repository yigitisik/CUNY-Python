...
def end_game():
    ...
    Popup("Game over", f"Your score was：{game.score}")
...
def button_click():
    ...
    if check_answer():
        ...
        Popup("Wrong answer", f"The answer was：{problem.answer}")
    if game.lives < 1:
        end_game()
    else:
        next_problem()
...
your_score_label = tkinter.Label(text=f"Your Score (スコア)：{game.score}", font=(FONT, 20, "normal"))
high_score_label = tkinter.Label(text=f"High Score (ハイスコア)：{game.high_score}", font=(FONT, 20, "normal"))

ones_entry = tkinter.Entry(width=1, font=ENTRY_FONT, justify=tkinter.RIGHT, textvariable=ones_value, validate="key",
                           validatecommand=validation)
tens_entry = tkinter.Entry(width=1, font=ENTRY_FONT, justify=tkinter.RIGHT, textvariable=tens_value, validate="key",
                           validatecommand=validation)
hundreds_entry = tkinter.Entry(width=1, font=ENTRY_FONT, justify=tkinter.RIGHT, textvariable=hundreds_value,
                               validate="key", validatecommand=validation)
thousands_entry = tkinter.Entry(width=1, font=ENTRY_FONT, justify=tkinter.RIGHT, textvariable=thousands_value,
                                validate="key", validatecommand=validation)

answer_button = tkinter.Button(text="Submit", font=(FONT, 20, "normal"), command=button_click)
